...
public class CartCombiner {
	...
	public static List<CartVO> combine(List<CartVO> originalCart, List<CartVO> inputCart) {
		...
		for (int i=0; i<originalCart.size()+inputCart.size(); i++) {
			addStock = originalCart.get(i).getQuantity(); // i의 수량을 기본값으로 초기화
			for (int j=i+1; j < originalCart.size()+inputCart.size(); j++) { // i의 다음 인덱스부터 중복값 검사
				// 시리얼과 사이즈가 같다면 같은 제품으로 봄
				if ( originalCart.get(i).getSerial() == originalCart.get(j).getSerial() && originalCart.get(i).getShoeSize() == originalCart.get(j).getShoeSize() ) {
